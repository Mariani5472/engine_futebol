...
export function TacticalPlayer({ player, x, y, selected, onSelect, onDragStart }: Props) {
  ...
  return (
    <button
      ...
      style={{ left: `${x}%`, top: `${y}%` }}
    >
      <span className={`flex h-11 w-11 items-center justify-center overflow-hidden rounded-full border-2 bg-background text-xs font-bold shadow-lg transition ${selected ? "border-primary ring-2 ring-primary/30" : "border-border"}`}>
        <img
          src={player.photoUrl}
          alt=""
