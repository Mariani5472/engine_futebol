...
type Props = { positions: TacticalPosition[]; playersById: Map<string, Athlete>; selectedPlayerId: string | null; onSelect: (playerId: string) => void; onMove: (playerId: string, x: number, y: number) => void };
...
export function TacticalPitch({ positions, playersById, selectedPlayerId, onSelect, onMove }: Props) {
  ...
  return <div className="rounded-xl border bg-card p-3 shadow-sm"><div ref={pitchRef} onDragOver={(event) => event.preventDefault()} onDrop={handleDrop} className="relative mx-auto aspect-[3/4] w-full max-w-[620px] overflow-hidden rounded-lg border-2 border-white/70 bg-emerald-700 shadow-inner">
    ...
    {positions.map((position) => { const player = playersById.get(position.playerId); if (!player) return null; return <TacticalPlayer key={position.playerId} player={player} x={position.x} y={position.y} selected={selectedPlayerId === position.playerId} onSelect={() => onSelect(position.playerId)} onDragStart={() => setDraggingPlayerId(position.playerId)} />; })}
  </div><p className="mt-3 text-center text-xs text-muted-foreground">Arraste os jogadores para qualquer ponto do campo.</p></div>;
}
