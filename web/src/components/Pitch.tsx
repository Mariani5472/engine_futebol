...
export function Pitch({ previous, current, alpha, layers }: PitchProps) {
  ...
  return <div className="pitch-shell"><div className="pitch" aria-label="Campo de futebol 2D">
    ...
    {current.players.map((player,index)=>{
      ...
      return <div className={`player ${player.team==="HOME"?"player-home":"player-away"} ${selectedId===player.id?"player-selected":""} ${owner?.id===player.id?"player-carrier":""}`} key={player.id} style={{left:`${point.x}%`,top:`${point.y}%`}} onClick={()=>setSelectedId(selectedId===player.id?null:player.id)} title={`${player.role??"Jogador"} ${player.number}`}>
        {layers.influence&&<i className="influence-area"/>}{layers.pressure&&underPressure&&<i className="pressure-area"/>}{player.hasBall&&<i className="possession-ring"/>}<span>{player.number}</span>{layers.roles&&<small className="role-label">{formatRole(player.role)}</small>}
      </div>;
    })}
