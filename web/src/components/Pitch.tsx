...
export interface PitchLayers {
  ...
  readonly enginePositions:boolean;
  readonly velocityVectors:boolean;
...
interface PitchProps {
  ...
  readonly onSelectPlayer?:(id:string|null)=>void;
}

export function Pitch({ previous, current, alpha, layers, selectedId=null, onSelectPlayer }: PitchProps) {
  ...
  return <div className="pitch-shell"><div className="pitch-stage"><div className="pitch" aria-label="Campo de futebol 2D" style={camera?{transformOrigin:`${camera.centerX}% ${camera.centerY}%`,transform:`scale(${camera.zoom})`}:undefined}>
    ...
    <svg className="pitch-overlay" viewBox="0 0 100 100" preserveAspectRatio="none" aria-hidden="true">
      ...
      {layers.velocityVectors&&current.players.map(player=>player.velocity&&<line key={`velocity-${player.id}`} x1={player.x} y1={player.y} x2={player.x+player.velocity.x*.5} y2={player.y+player.velocity.y*.7} className="velocity-line"/>)}
      {layers.orientation&&current.players.map(player=>player.facingDirection&&<line key={`facing-${player.id}`} x1={player.x} y1={player.y} x2={player.x+player.facingDirection.x*2} y2={player.y+player.facingDirection.y*3} className="orientation-line"/>)}
    ...
    {current.players.map((player,index)=>{
      ...
      return <div className={`player ${player.team==="HOME"?"player-home":"player-away"} ${selectedId===player.id?"player-selected":""} ${owner?.id===player.id?"player-carrier":""}`} key={player.id} style={{left:`${point.x}%`,top:`${point.y}%`}} onClick={()=>onSelectPlayer?.(selectedId===player.id?null:player.id)} title={`${player.role??"Jogador"} ${player.number}${player.goalkeeperState?` · ${player.goalkeeperState}`:player.tacticalResponsibility?` · ${player.tacticalResponsibility}`:""}`}>
    ...
    {layers.enginePositions&&current.players.map(player=><div key={`engine-${player.id}`} className="engine-position" style={{left:`${player.x}%`,top:`${player.y}%`}} title={`Engine ${player.id}`}/>)}
  </div></div></div>;
}
