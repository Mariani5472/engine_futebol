...
export type TrainingScenarioId = FundamentalScenarioSkill | CurriculumScenarioStage;

export const INDIVIDUAL_TRAINING_STAGE_ORDER: readonly FundamentalScenarioSkill[] = Object.freeze([
  "MOVEMENT", "BALL_CONTROL", "PASSING", "SHOOTING_EMPTY_GOAL",
]);
...
export const SELF_TRAINING_STAGE_ORDER: readonly TrainingScenarioId[] = Object.freeze([
  ...INDIVIDUAL_TRAINING_STAGE_ORDER,
  ...LEGACY_CURRICULUM_STAGE_ORDER,
]);

export interface TrainingScenarioDefinition {
  readonly id: TrainingScenarioId;
...
interface DefinitionInput extends Omit<TrainingScenarioDefinition, "version"> {}
...
const curriculum = (input: Omit<DefinitionInput, "scenario"> & { readonly id: CurriculumScenarioStage }) =>
  define({ ...input, scenario: createCurriculumScenarioPreset(input.id) });

const fundamental = (
  input: Omit<DefinitionInput, "scenario"> & { readonly id: FundamentalScenarioSkill },
  scenario: Omit<FundamentalScenarioConfig, "kind" | "version" | "skill">,
) => define({
  ...input,
  scenario: { kind: "FUNDAMENTAL", version: FUNDAMENTAL_SCENARIO_VERSION, skill: input.id, ...scenario },
});

const CATALOG: readonly TrainingScenarioDefinition[] = Object.freeze([
  fundamental({
    id: "MOVEMENT", family: "MOVEMENT", controlledAgentIds: ["home-10"], policyMode: "SINGLE_AGENT", opponentMode: "NONE",
    difficulty: { distanceMeters: 8, targetRadiusMeters: 0.6 }, decisionIntervalTicks: 20, timeLimitSeconds: 12,
    terminationRules: ["TARGET_REACHED", "TIME_LIMIT"], successRules: ["TARGET_REACHED"],
    rewardProfileId: "FUNDAMENTAL_V1", requiredCapabilities: ["MOVE", "STEERING_LOCOMOTION"],
  }, { playerId: "home-10", playerPosition: { x: 40, y: 34 }, targetPosition: { x: 48, y: 34 }, ballPosition: { x: 1, y: 1 }, targetRadius: 0.6 }),
  fundamental({
    id: "BALL_CONTROL", family: "BALL_CONTROL", controlledAgentIds: ["home-10"], policyMode: "SINGLE_AGENT", opponentMode: "NONE",
    difficulty: { ballDistanceMeters: 2 }, decisionIntervalTicks: 20, timeLimitSeconds: 8,
    terminationRules: ["BALL_CONTROLLED", "BALL_OUT", "TIME_LIMIT"], successRules: ["BALL_CONTROLLED"],
    rewardProfileId: "FUNDAMENTAL_V1", requiredCapabilities: ["CONTROL", "PHYSICAL_POSSESSION"],
  }, { playerId: "home-10", playerPosition: { x: 50, y: 34 }, ballPosition: { x: 52, y: 34 } }),
  fundamental({
    id: "PASSING", family: "PASSING", controlledAgentIds: ["home-10"], policyMode: "SINGLE_AGENT", opponentMode: "NONE",
    difficulty: { passDistanceMeters: 10 }, decisionIntervalTicks: 20, timeLimitSeconds: 12,
    terminationRules: ["PASS_COMPLETED", "PASS_INTERCEPTED", "BALL_OUT", "TIME_LIMIT"], successRules: ["PASS_COMPLETED"],
    rewardProfileId: "FUNDAMENTAL_V1", requiredCapabilities: ["PASS", "PHYSICAL_RECEPTION"],
  }, { playerId: "home-10", playerPosition: { x: 45, y: 34 }, receiverId: "home-9", receiverPosition: { x: 55, y: 34 } }),
  fundamental({
    id: "SHOOTING_EMPTY_GOAL", family: "SHOOTING", controlledAgentIds: ["home-10"], policyMode: "SINGLE_AGENT", opponentMode: "NONE",
    difficulty: { distanceFromGoalMeters: 9 }, decisionIntervalTicks: 20, timeLimitSeconds: 8,
    terminationRules: ["SHOT_RESOLVED", "BALL_OUT", "TIME_LIMIT"], successRules: ["GOAL"],
    rewardProfileId: "FUNDAMENTAL_V1", requiredCapabilities: ["SHOT", "BALL_PHYSICS", "EMPTY_GOAL"],
  }, { playerId: "home-10", playerPosition: { x: 96, y: 34 } }),
  curriculum({
...
export function trainingScenarioDefinition(id: TrainingScenarioId): TrainingScenarioDefinition {
  const definition = BY_ID.get(id);
  if (!definition) throw new Error(`Unknown training scenario: ${id}`);
  return definition;
}

export function curriculumTrainingScenarioDefinition(id: CurriculumScenarioStage): TrainingScenarioDefinition & { readonly scenario: CurriculumScenarioConfig } {
  const definition = trainingScenarioDefinition(id);
  if (definition.scenario.kind !== "CURRICULUM") throw new Error(`${id} is not a curriculum scenario`);
  return definition as TrainingScenarioDefinition & { readonly scenario: CurriculumScenarioConfig };
}
...
function validateCatalog(catalog: readonly TrainingScenarioDefinition[]): void {
  ...
  for (const definition of catalog) {
    ...
    const scenarioId = definition.scenario.kind === "CURRICULUM" ? definition.scenario.stage
      : definition.scenario.kind === "FUNDAMENTAL" ? definition.scenario.skill
      : "ATTACKER_VS_GOALKEEPER";
    if (scenarioId !== definition.id) throw new Error(`Scenario ${definition.id} has a mismatched preset`);
